 ...
 * by extracting text from the FAQ answer content
 */
export const generateMetaDescription: CollectionBeforeChangeHook = async ({ data, operation }) => {
  ...
  if ((operation === 'create' || operation === 'update') && data.meta) {
    ...
    if (!data.meta.description) {
      // Try to create a description from the answer content if available
      if (data.answer) {
        try {
          ...
          if (data.answer.root?.children) {
            for (const child of data.answer.root.children) {
      ...
      if (!data.meta.description && data.question) {
        data.meta.description = `Find the answer to "${data.question}" in our FAQ section.`
      }
    }
  }
